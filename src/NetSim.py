    ...
    def run(self) -> None:
        ...
        while self._event_queue:
            try:
                ...
                if time > self.max_time: break
                if b_step_log: print(f"[{time:.3f}s] ▶ 이벤트 발생: {event.handler.__name__}({event.args}, {event.kwargs})")
...
def launch_simulation() -> None:
    ...
    avg_speed_var = tk.DoubleVar(value=40.0)
    outage_zone_var = tk.DoubleVar(value=800.0)
    num_blocks_var = tk.IntVar(value=5)
    density_var = tk.DoubleVar(value=10.0)
    reroute_var = tk.BooleanVar(value=False)
    step_log_var = tk.BooleanVar(value=False)
